...
lokacije = []
for (mesto, drzava), število in st_eventov.items():
    koordinata = pridobi_koord(mesto, drzava)
    if koordinata:
        lon, lat = koordinata
        lokacije.append((lon, lat, število))
...
for lon, lat, stevilo in lokacije:
    ax.scatter(lon, lat, s=stevilo*15, color='blue', alpha=0.6, edgecolors='black')

plt.title('UFC dogodki po svetu (velikost krogca = število eventov)')
plt.axis('off')
plt.tight_layout()
plt.savefig(r'C:\Users\mojca\OneDrive\Namizje\PROG\python_projektna\zemljevid_dogodkov.png')
